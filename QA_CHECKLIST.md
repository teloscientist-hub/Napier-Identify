...
- [ ] Prototype API mode can switch between mock, real, and failure.
...
- [x] Mock mode still returns ranked candidates.
- [x] Real mode explains missing `EXPO_PUBLIC_NAPIER_API_BASE_URL` when no backend URL is configured.
- [x] Real mode can call `POST /identify` when a test backend URL is configured.
- [x] API candidates render in Results.
- [x] API no-match response renders retry guidance.
- [ ] API server/network error state does not dead-end the user.
- [x] Opening a candidate fetches item detail in real mode.
- [ ] Missing optional item fields display fallback text instead of crashing.
- [ ] Failure mode still forces identify and item-detail error states.
